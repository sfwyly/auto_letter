...
def train():
    ...
    # model = get_model()
    model = load_model('./checkpoints/{}.h5'.format('love'))
...
class MyModel:
    def __init__(self, model_id='love'):
        self.model = load_model('./checkpoints/{}.h5'.format(model_id))
        self.data, self.word2id, self.id2word = get_data()

    def __call__(self, life, live):

        prefix, start = live, life
        start = ["<START>"] + list(prefix) + list(start)
        # word2id
        token_ids = [self.word2id[w] for w in start]

        while len(token_ids) < config.max_gen_len:
            output = self.model(np.array([token_ids, ], dtype=np.int32))
            prob = output.numpy()[0, -1, :]
            del output
            # 按照出现概率，对所有token倒序排列，取最高概率的字
            target = prob.argsort()[::-1][0]
            ...
            # 保存
            if target == self.word2id['<EOP>']:
                break
            else:
                token_ids.append(target)
        result = [self.id2word[w] for w in token_ids[len(prefix) + 1:]]
        return ''.join(result)


def hold_palm(model_id):
